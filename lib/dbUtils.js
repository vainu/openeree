...
function _search(req) {
    ...
    if (search) {
        let searchKeys = search.split(';');

        searchKeys.forEach(function (s) {
            if (s.includes('=')) {
                outS.push(s.split('='));
            } else {
                let likeSearch = s.split(':');

                outS.push([likeSearch[0], 'like', '%'+ decodeURI(likeSearch[1]) +'%']);
            }
        });
    }
...
function applyTransformations(q, req) {
    ...
    _search(req).forEach(function (s) {
        console.log(s);
        if (s.length == 2) {
            q.where(s[0], s[1]);
        } else {
